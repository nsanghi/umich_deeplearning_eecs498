...
def compute_distances_no_loops(x_train, x_test):
  ...
  dists = (torch.sum(x_train1**2,dim=1, keepdim=True) + 
    torch.sum(x_test1**2,dim=1, keepdim=True).T 
    -2*torch.mm(x_train1, x_test1.T))
  #dists = torch.sum((x_train.view(num_train, 1, -1)-x_test.view(1, num_test, -1))**2, dim=2)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  values,indices = dists.topk(k,dim=0, largest=False)
  for i in range(num_test):
    idxs = indices[:,i]
    best_label,_ = torch.mode(y_train[idxs])
    y_pred[i] = best_label
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    ...
    for n in range(num_folds):
      x_train_cv = torch.cat(x_train_folds[:n] + x_train_folds[n+1:])
      y_train_cv = torch.cat(y_train_folds[:n] + y_train_folds[n+1:])
      x_test_cv, y_test_cv = x_train_folds[n], y_train_folds[n]
      classifier = KnnClassifier(x_train_cv, y_train_cv)
      acc.append(classifier.check_accuracy(x_test_cv, y_test_cv, k=k, quiet=True))
...
def knn_get_best_k(k_to_accuracies):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  k_to_mean_acc = [(k, statistics.mean(v)) for k,v in k_to_accuracies.items()]
  k_to_mean_acc.sort(key = lambda x: x[1])
  best_k = k_to_mean_acc[0][0]
