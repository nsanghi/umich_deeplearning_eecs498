...
class Linear(object):
  ...
  def forward(x, w, b):
    ...
    N = x.shape[0]
    flat_x = x.contiguous().reshape(N,-1)
    out = flat_x.mm(w)+b 
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
  ...
  def backward(dout, cache):
    ...
    dw = x.view(dout.shape[0],-1).T.mm(dout)
    db = dout.sum(dim=0)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    scores_layer1, cache_layer1 = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])
    scores, cache_layer2 = Linear_ReLU.forward(scores_layer1, self.params['W2'], self.params['b2'])
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
    ...
    loss += self.reg * (self.params['W2']**2).sum()
    dout, dw2, db2 = Linear_ReLU.backward(dout, cache_layer2)
    dx, dw1, db1 = Linear_ReLU.backward(dout, cache_layer1)
    grads['W2'] = dw2 + self.reg * 2 * self.params['W2']
    grads['b2'] = db2
    grads['W1'] = dw1 + self.reg * 2 * self.params['W1']
    grads['b1'] = db1
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
  ...
  def loss(self, X, y=None):
    ...
    for i in range(1, self.num_layers+1):
      ...
      if i == self.num_layers:
        ...
        if self.use_dropout and mode == 'train':
          scores, cache_d = Dropout.forward(scores, self.dropout_param)
          cache_dict[f'{i}_d'] = cache_d
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    for i in range(self.num_layers, 0, -1):
      ...
      if i == self.num_layers:
        ...
        if self.use_dropout and mode == 'train':
          dout = Dropout.backward(dout, cache_dict[f'{i}_d'])
        dout, dw, db = Linear_ReLU.backward(dout, cache_dict[i])
...
def rmsprop(w, dw, config=None):
  ...
  # Replace "pass" statement with your code
  cache = config['cache']
  cache.mul_(config['decay_rate'])
  cache.add_((1-config['decay_rate'])*(dw**2))
  config['cache'] = cache
...
class Dropout(object):
  ...
  def forward(x, dropout_param):
    ...
    if mode == 'train':
      ...
      ###########################################################################
      # Replace "pass" statement with your code
      mask = (torch.rand_like(x) > p) / (1-p) #mask stores the neurons allowed to flow
      out = x * mask
      ###########################################################################
      #                             END OF YOUR CODE                            #
      ###########################################################################
