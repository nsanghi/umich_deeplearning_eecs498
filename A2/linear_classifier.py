...
def svm_loss_vectorized(W, X, y, reg):
  ...
  margin = torch.clamp(scores-correct_class_scores+1.0, 0.0)
  margin[torch.arange(num_train), y] = 0.0
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  train_acc = (cls.predict(X_train) == y_train).float().mean()  
  val_acc = (cls.predict(X_val) == y_val).float().mean()  
  ############################################################################
  #                            END OF YOUR CODE                              #
  ############################################################################
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    ...
    dW[:,y[i]] -= X[i]
    dW += torch.matmul(torch.unsqueeze(X[i], 1), torch.unsqueeze(prob, 0))
  loss /= num_train
  loss += reg * torch.sum(W*W)
  dW /= num_train
  dW += 2 * reg * W
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  loss += -torch.log(prob[torch.arange(num_train), y]).mean()
  loss += reg * torch.sum(W*W)
  correct_class = torch.zeros((num_train, num_classes), dtype=X.dtype, device=X.device)
  correct_class[torch.arange(num_train), y] = -1
  dW += torch.mm(X.T, prob+correct_class)
  dW /= num_train
  dW += 2 * reg * W
  #############################################################################
  #                          END OF YOUR CODE                                 #
