...
def nn_forward_pass(params, X):
    ...
    hidden = torch.clip(X.mm(W1)+b1, 0.0)
...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    dScores = probs
    dScores[range(N), y] += -1
    grads['b2'] = dScores.mean(dim=0)
    grads['W2'] = h1.t().mm(dScores) / N + 2 * reg * W2
    dh1 = dScores.mm(W2.T)
    do1 = dh1 * (X.mm(W1)+b1 > 0) 
    grads['b1'] = do1.mean(dim=0)
    grads['W1'] = X.t().mm(do1) / N + 2 * reg * W1
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    ...
    for learning_rate_decay in learning_rate_decays:
      for learning_rate in learning_rates:
        for reg in regularization_strengths:
          print('train with hidden_size: {}, lr_decay:{}, lr:{}, reg: {}'.format(hidden_size, learning_rate_decay, learning_rate, reg))
          net = TwoLayerNet(D, hidden_size, C, device=X.device, dtype=X.dtype)
          stat = net.train(X, y, X_val, y_val,
                        num_iters=3000, batch_size=1000,
                        learning_rate=learning_rate, 
                        learning_rate_decay=learning_rate_decay,
                        reg=reg)
          val_acc = stat['val_acc_history'][-1] #pick last val acc from history
